...
export function empty(
  ...
  if (type === 'ZodTuple') {
    return spec._def.items.map((s: z.ZodTypeAny) => empty(s, options))
  }

  if (type === 'ZodObject') {
    const shape = spec._def.shape()
    return Object.entries(shape).reduce((acc: any, [key]: any) => {
      acc[key] = empty(shape[key], options)
      return acc
    }, {})
  }
  ...
  if (type === 'ZodUnion') {
    ...
    return empty(types[one], options)
  }
  if (type === 'ZodIntersection') {
    return {
      ...empty(spec._def.left, options),
      ...empty(spec._def.right, options),
    }
  }
