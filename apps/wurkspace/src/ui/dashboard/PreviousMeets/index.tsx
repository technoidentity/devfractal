...
export const getReportingToInfo = (reportingTo: string) => {
  const [firstName, lastName, managerId] = reportingTo.split(' ')
  return { firstName, lastName, managerId }
}

export const fullName = (employee: Employee): string =>
  `${employee.firstName} ${employee.lastName}`

export const getBoard = (userEmail: string, meeting: PreviousMeeting) => {
  const [me, you] =
    userEmail === meeting.calendarEvent.attendee1Email
      ? [meeting.calendarEvent.attendee1, meeting.calendarEvent.attendee2]
      : [meeting.calendarEvent.attendee2, meeting.calendarEvent.attendee1]

  return {
    me: fullName(me),
    you: fullName(you),
    isManager: me.managerEmail === null ? false : me.managerEmail === you.email,
  }
}

export const PreviousMeets = () => {
  const [session] = useAuth()
  const userEmail = session.user!.email!
  const { meetings, error } = usePreviousMeetings()

  return (
    <div className="font-nunito">
      <div className="flex flex-col bg-white rounded-lg h-60 shadow-lg lg:w-99 max-w-md sm:py-3 sm:px-4 px-5 pt-3">
        <h1 className="lg:text-2xl sm:text-xl text-xl font-bold pb-1">
          Your Boards
        </h1>

        {/*  Display Previous meeting cards */}
        <div className="scrollbar h-44 overflow-hidden overflow-y-scroll">
          <div className="flex flex-col justify-center pr-2">
            {/* Checking if the meetings is existed or not */}
            {meetings ? (
              // checking user has previous meeting Info or not
              meetings.length ? (
                // call the Card comp if the meetings exist
                meetings?.map(meeting => {
                  return (
                    <div key={meeting.id} className="py-1">
                      <Card
                        meeting={meeting}
                        userEmail={userEmail}
                        eventId={meeting.id}
                ...
                // If we don't have previous meetings
                <div className="-ml-3">
                  <div className="flex justify-center items-center h-[140px] w-[415px] text-2xl font-extralight">
                    No boards
                  </div>
                </div>
              )
            ) : error ? (
              <ErrorMessage error="can't fetch previous meetings" />
            ) : (
              // Showing loading indication when data is Loading...
              <div className="h-[140px] w-[415px] flex justify-center items-center">
                <LoadingScreen />
              </div>
            )}
          </div>
...
const Card = ({ userEmail, meeting }: CardProps) => {
  const { me, you, isManager } = getBoard(userEmail, meeting)

  return (
    <div className="bg-gray-100 w-full flex rounded-md pl-4">
