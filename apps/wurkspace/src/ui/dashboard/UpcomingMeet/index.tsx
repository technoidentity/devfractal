...
type CardsProps = { meetings: UpComingMeetingsResponse }

const Loading = () => (
  <div className="h-[130px] w-[415px] flex justify-center items-center">
    <LoadingScreen />
  </div>
)
...
const EmptyMeetings = () => (
  <div className="-ml-3">
    <div className="flex justify-center items-center h-[110px] w-[415px] text-2xl font-extralight">
      No meeting scheduled
    </div>
  </div>
)

const Cards = ({ meetings }: CardsProps) => (
  <div className="grid grid-cols-3 gap-1 ">
    {meetings.map((item: UpComingMeeting) => (
      <div
        key={item.id}
        className={`py-4 sm:py-3 bg-gray-100 rounded-md text-center ${
          item.calendarEvent?.attendee2ResponseStatus === 'needsAction' &&
          'cursor-not-allowed'
        }`}
      >
        <div>
...
const Footer = () => (
  <div className="flex flex-row space-x-4 font-semibold">
    <div className="inline-flex space-x-2">
      <div className="mt-3 w-2 h-2 bg-green-550 rounded-full "></div>
      <div className="m-1">Your Manager</div>
    </div>
    <div className="inline-flex space-x-2">
      <div className="mt-3 w-2 h-2 bg-yellow-500 rounded-full "></div>
      <div className="m-1">Other</div>
    </div>
  </div>
)

export const UpcomingMeet = () => {
  const { meetings, error } = useUpcomingMeets()

  return (
    <div className=" font-nunito">
      <div className="bg-white shadow-xl rounded-2lg h-60 max-w-md lg:px-6 sm:py-3 sm:px-4 px-4 pt-3">
        <div className="lg:text-2xl sm:text-xl text-xl font-semibold leading-1 text-gray-800 pb-2">
          Your upcoming One-on-one’s
        </div>

        <div>
          <div className="overflow-x-scroll h-scrollbar overflow-y-scroll scrollbar h-[130px] w-[415px]">
            <div className="font-semibold sm:-ml-1 sm:py-1 pr-2">
              {!meetings ? (
                error ? (
                  <ErrorMessage error="can't fetch meetings" />
                ) : (
                  <Loading />
                )
              ) : meetings.length === 0 ? (
        ...
        {/* detailed info of the green and yellow color shown */}
        <Footer />
      </div>
    </div>
  )
...
const Card = (props: any) => {
  const { id, startTime, calendarEvent } = props

  const status = calendarEvent.attendee1ResponseStatus

  const otherType =
    calendarEvent.attendee1.email === calendarEvent.attendee2.managerEmail
      ? 'Manager'
      : 'Other'

  // React.useEffect(() => {
  //   if (
  //     userInfo?.data.Reporting_To ===
  //     `${otherInfo?.data.FirstName} ${otherInfo?.data.LastName} ${otherInfo?.data.EmployeeID}`.toString()
  //   ) {
  //     setUserType('Manager')
  //   }
  // }, [
  //   otherInfo?.data.EmployeeID,
  //   otherInfo?.data.FirstName,
  //   otherInfo?.data.LastName,
  //   userInfo?.data.Reporting_To,
  // ])
  return (
    <Link href={`/one-on-one-new/${encodeURIComponent(id)}`} passHref>
      <div className="flex flex-col items-center w-32 h-24 ">
        {calendarEvent.attendee2.managerEmail !== null && (
          <div>
            <svg
              width="29"
              height="29"
        ...
        <div
          className={`text-base pt-1 px-2 leading-[15px] ${
            status === 'accepted'
              ? otherType === 'Manager'
                ? 'text-green-550'
                : 'text-yellow-550'
        ...
        >
          <h1 className={`${status === 'needsAction' && 'cursor-not-allowed'}`}>
            {calendarEvent.attendee1.firstName}
          </h1>
        </div>
        <div
          className={`lg:px-3 px-4 pt-2 text-xs sm:text-xs lg:text-xs ${
            status === 'accepted' ? 'text-gray-900 font-bold' : 'text-gray-400'
          }`}
        >
