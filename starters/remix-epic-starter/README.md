...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-epic-starter-40d9
  fly apps create remix-epic-starter-40d9-staging
  ```
...
- Add a `SESSION_SECRET`, `ENCRYPTION_SECRET`, and `INTERNAL_COMMAND_TOKEN` to
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app remix-epic-starter-40d9
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) ENCRYPTION_SECRET=$(openssl rand -hex 32) INTERNAL_COMMAND_TOKEN=$(openssl rand -hex 32) --app remix-epic-starter-40d9-staging
  ```
...
- **Create an account on Mailgun.** (Can be deferred to later)
  ...
  ```sh
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app remix-epic-starter-40d9
  fly secrets set MAILGUN_DOMAIN="mg.example.com" MAILGUN_SENDING_KEY="some-api-token-with-dashes" --app remix-epic-starter-40d9-staging
  ```
...
- Create a persistent volume for the sqlite database for both your staging and
  ...
  ```sh
  fly volumes create data --region sjc --size 1 --app remix-epic-starter-40d9
  fly volumes create data --region sjc --size 1 --app remix-epic-starter-40d9-staging
  ```
